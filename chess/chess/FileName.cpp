...
bool Tablero::esMovimientoLegal(int fo, int co, int fd, int cd) const {
    ...
    case TipoPieza::PEON: {
        ...
        if (dc == 0 && df == 2 * dir && d.tipo == TipoPieza::VACIO &&
            ((o.color == ColorPieza::BLANCO && fo == 6) ||
                (o.color == ColorPieza::NEGRO && fo == 1)))
            return caminoLibre(fo, co, fd, cd);
        if (std::abs(dc) == 1 && df == dir && d.tipo != TipoPieza::VACIO)
            return true;
        return false;
    }
    ...
    case TipoPieza::CABALLO:
        return (std::abs(df) == 2 && std::abs(dc) == 1) ||
            (std::abs(df) == 1 && std::abs(dc) == 2);
    case TipoPieza::ALFIL:
        return (std::abs(df) == std::abs(dc)) && caminoLibre(fo, co, fd, cd);
    case TipoPieza::REINA:
        return ((df == 0 || dc == 0) || (std::abs(df) == std::abs(dc))) &&
            caminoLibre(fo, co, fd, cd);
    case TipoPieza::REY:
...
bool Tablero::moverPieza(int fo, int co, int fd, int cd) {
    ...
    turnoActual = (turnoActual == ColorPieza::BLANCO)
...
int main() {
    ...
    while (true) {
        system("cls"); // usa "clear" en Linux/Mac
        tablero.imprimirTablero();

        std::cout << "Turno: "
            << ((tablero.turnoActual == ColorPieza::BLANCO) ? "BLANCAS" : "NEGRAS")
            << std::endl;
        ...
        int fo, co, fd, cd;
        if (!coordenadasAIndices(cmd.substr(0, 2), fo, co) ||
            !coordenadasAIndices(cmd.substr(2, 2), fd, cd))
            continue;
